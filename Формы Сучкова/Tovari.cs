                ...
                if (dataGridView1.Rows[i].Cells[0].Value != null)
                {
                    if (dataGridView1.Rows[i].Cells[0].Value.ToString() == "true")
                    {
                        ...
                        else
                        {
                            ...
                            cmd1.CommandText = "SELECT pk_tab, value, pkprod, pk_char from table_conform where pk_prod = " + dataGridView1.Rows[i].Cells[9].Value.ToString(); ;
                            dr1 = cmd1.ExecuteReader();

                            while (dr1.Read())
                            {
                                list_elemConfTable.Add(new elemOfConfTable((int)dr2[0], dr2[1].ToString(), (int)dr2[2], (int)dr2[3]));
                            }

                            cmd1.CommandText = "delete from table_conform where product.pk_prod = " + dataGridView1.Rows[i].Cells[9].Value.ToString();
